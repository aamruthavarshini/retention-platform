  ...
  fs.createReadStream(req.file.path)
    ...
    .on('end', () => {
      try {
        ...
        // Auto-seed retention actions for at-risk customers
        const atRiskCustomers = db.prepare(
          "SELECT * FROM customers WHERE risk_score >= 0.4 AND status = 'Active'"
        ).all();
        ...
        const seedActions = db.transaction((customers) => {
          for (const c of customers) {
            const idx = Math.floor(Math.random() * actions.length);
            insertAction.run(c.customer_id, actionTypes[idx], actions[idx], 'Pending', new Date().toISOString());
          }
        });

        seedActions(atRiskCustomers);
        ...
        res.json({
          ...
          autoCalculated,
          actionsSeeded: atRiskCustomers.length,
        });
